"""sklearn_random_forest_baseline.py — baseline cell for tabular classification.

Trains a RandomForestClassifier from scikit-learn. Good for quick sanity checks.
Saves `model.pkl` under `mlops/models/<scenario>/vN/`.
"""
...
def run(ctx: BackboneContext, prev: list[Any]) -> dict[str, Any]:
    ...
    n_estimators = int(bcfg.get("n_estimators") or 300)
    max_depth_raw = bcfg.get("max_depth")
    max_depth = int(max_depth_raw) if isinstance(max_depth_raw, int) or str(max_depth_raw).isdigit() else None
    if task == "regression":
        ...
        pred = rf.predict(x_val) if y_val.size else np.array([], dtype=np.float32)
        ...
        rf.fit(x_train, y_train)
        val_metric = float((rf.predict(x_val) == y_val).mean()) if y_val.size else 0.0  # ACC
    ...
    if task == "regression":
        summary = f"trained RandomForest(regression) → {run_dir.name} (val_mae={metrics['val_metric']:.4f})"
    else:
        summary = f"trained RandomForest(classification) → {run_dir.name} (val_acc={metrics['val_metric']:.4f})"
    print(summary)
    return {
        "output": summary,
        "data": {
