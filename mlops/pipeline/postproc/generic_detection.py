...
def run(image_bgr: np.ndarray, raw_detections: list[dict[str, Any]], config: dict[str, Any]) -> dict[str, Any]:
    """Generic detection postproc.

    Draws boxes above a confidence threshold and returns a consistent payload shape.
    """
    hyperparams = config.get("hyperparams") if isinstance(config, dict) else {}
    threshold_raw = (hyperparams or {}).get("confidence_threshold", 0.25)
    try:
    ...
    return {
        "signal": {
            "flag": bool(detections),
            "summary": f"{len(detections)} detection(s) >= {threshold:.2f}" if detections else "no detections",
            "metrics": {
